...
var FetchableDevEnvironment = class extends DevEnvironment {
	...
	async fetchModule(id, importer, options) {
		if (this.#preventExternalize && WORKERD_BUILTIN_RE.test(id)) return {
			externalize: id,
			type: "builtin"
		};
...
async function configureViteDevServer(ctx, server) {
	...
	server.middlewares.use(function nitroDevMiddlewarePre(req, res, next) {
		...
		const pathname = new URL(withBase(req.url, nitro.options.baseURL), "http://localhost").pathname;
		const match = nitro.routing.routes.match(req.method || "", pathname);
		const matchedHandlers = match ? Array.isArray(match) ? match : [match] : [];
		const isExplicitRoute = matchedHandlers.some((h) => h?.route && h.route !== "/**" && !h.route.startsWith("/**:"));
		const isExplicitPublicAsset = nitro.options.publicAssets.some((asset) => asset.baseURL && asset.baseURL !== "/" && !asset.fallthrough && (pathname === asset.baseURL || pathname.startsWith(asset.baseURL + "/")));
		if (isExplicitRoute || isExplicitPublicAsset) return nitroDevMiddleware(req, res, next);
